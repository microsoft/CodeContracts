...
{
    internal class Prune : MetadataRewriter
    {
        [ContractInvariantMethod]
        private void ObjectInvariant()
        {
            Contract.Invariant(_thingsToKeep != null);
        }


        readonly private HashSet<object> _thingsToKeep;
        readonly private MetadataDeepCopier _copier;
        readonly private HashSet<uint> _methodDefsToKeep;
        private IDictionary<IMethodDefinition, uint> _contractOffsets;
        readonly private IDictionary<IMethodDefinition, MethodHashAttribute> _methodHashAttributes;


        readonly private INamespaceTypeReference _systemString;
        readonly private INamespaceTypeReference _systemInt;
        private MethodReference _methodHashAttributeCtor;
          ...
          : base(host)
        {
            ...
            _methodDefsToKeep = methodsWhoseBodiesShouldBeKept;
            _methodHashAttributes = methodHashAttributes;

            _systemInt = host.PlatformType.SystemInt32;
            _systemString = host.PlatformType.SystemString;
        }
        ...
        public static Assembly PruneAssembly(HostEnvironment host, ISlice<MethodReferenceAdaptor, FieldReferenceAdaptor, TypeReferenceAdaptor, IAssemblyReference> slice)
        {
            ...
            me._contractOffsets = contractOffsets;
            ...
            me._methodHashAttributeCtor = new Microsoft.Cci.MethodReference(
                host,
        ...
        private void VisitChain(IChain<MethodReferenceAdaptor, FieldReferenceAdaptor, TypeReferenceAdaptor> c)
        {
            ...
            switch (c.Tag)
            {
                case ChainTag.Type:
                    ...
                    break;
                case ChainTag.Field:
                    _thingsToKeep.Add(c.Field.reference.ResolvedField);
                    break;
                case ChainTag.Method:
                    var m = c.Method.reference.ResolvedMethod;
                    _thingsToKeep.Add(m);
                    _methodHashAttributes.Add(m, c.MethodHashAttribute);
                    break;
                default:
        ...
        {
            _thingsToKeep.Add(iUnitNamespaceReference);
        ...
        {
            Contract.Assume(_copier.OriginalFor.ContainsKey(unitNamespace));
            var origNs = _copier.OriginalFor[unitNamespace];
            Contract.Assume(_thingsToKeep.Contains(origNs));

            var members_prime = new List<INamespaceMember>();
            foreach (var m in unitNamespace.Members)
            {
                Contract.Assume(_copier.OriginalFor.ContainsKey(m));
                var orig = _copier.OriginalFor[m];
                if (_thingsToKeep.Contains(orig))
                {
                    members_prime.Add(m);
                    base.Rewrite(m);
                }
        ...
        public override void RewriteChildren(NamedTypeDefinition typeDefinition)
        {
            ...
            Contract.Assume(_copier.OriginalFor.ContainsKey(typeDefinition));
            var origTypeDefinition = _copier.OriginalFor[typeDefinition];
            Contract.Assume(_thingsToKeep.Contains(origTypeDefinition));
            ...
            if (typeDefinition.Methods != null)
            {
                ...
                {
                    Contract.Assume(_copier.OriginalFor.ContainsKey(m));
                    var orig = (IMethodDefinition)_copier.OriginalFor[m];
                    if (_thingsToKeep.Any(ttk => { var m2 = ttk as IMethodReference; return m2 != null && m2.InternedKey == orig.InternedKey; }))
                    {
                        methods_prime.Add(m);
                        this.Rewrite(m);
                    }
            ...
            if (typeDefinition.Fields != null)
            {
                ...
                {
                    Contract.Assume(_copier.OriginalFor.ContainsKey(f));
                    var orig = (IFieldDefinition)_copier.OriginalFor[f];
                    if (_thingsToKeep.Any(ttk => { var f2 = ttk as IFieldReference; return f2 != null && f2.InternedKey == orig.InternedKey; }))
                    {
                        fields_prime.Add(f);
                    }
                }
            ...
            if (typeDefinition.NestedTypes != null)
            {
                ...
                {
                    Contract.Assume(_copier.OriginalFor.ContainsKey(nt));
                    var orig = (INestedTypeDefinition)_copier.OriginalFor[nt];
                    if (_thingsToKeep.Any(ttk => { var nt2 = ttk as INestedTypeReference; return nt2 != null && nt2.InternedKey == orig.InternedKey; }))
                    {
            ...
            if (typeDefinition.Properties != null)
            {
                ...
                {
                    Contract.Assume(_copier.OriginalFor.ContainsKey(p));
                    var orig = _copier.OriginalFor[p];
                    var keep = _thingsToKeep.Contains(orig); // REVIEW: How to do this without relying on object identity? prop defs don't have interned keys.
                    ...
                    var keepGetter = originalProperty.Getter != null && _thingsToKeep.Any(ttk => { var m = ttk as IMethodReference; return m != null && m.InternedKey == originalProperty.Getter.InternedKey; });
                    var keepSetter = originalProperty.Setter != null && _thingsToKeep.Any(ttk => { var m = ttk as IMethodReference; return m != null && m.InternedKey == originalProperty.Setter.InternedKey; });
        ...
        {
            Contract.Assume(_copier.OriginalFor.ContainsKey(propertyDefinition));
            var orig = _copier.OriginalFor[propertyDefinition];
            var originalProperty = (IPropertyDefinition)orig;
            if (originalProperty.Getter != null && !_thingsToKeep.Contains(originalProperty.Getter.ResolvedMethod))
            {
                propertyDefinition.Getter = null;
                var originalGetter = (IMethodDefinition)_copier.CopyFor[originalProperty.Getter.ResolvedMethod];
                propertyDefinition.Accessors.Remove(originalGetter);
            }
            if (originalProperty.Setter != null && !_thingsToKeep.Contains(originalProperty.Setter.ResolvedMethod))
            {
                propertyDefinition.Setter = null;
                var originalSetter = (IMethodDefinition)_copier.CopyFor[originalProperty.Setter.ResolvedMethod];
        ...
        public override void RewriteChildren(MethodDefinition method)
        {
            ...
            Contract.Assume(_copier.OriginalFor.ContainsKey(method));
            var originalMethod = (IMethodDefinition)_copier.OriginalFor[method];
            if (_methodHashAttributes.TryGetValue(originalMethod, out mhAttr))
            {
                args = new List<IMetadataExpression>{
            new MetadataConstant(){ Type = _systemString, Value = mhAttr.Arg1, },
            new MetadataConstant(){ Type = _systemInt, Value = mhAttr.Arg2, },
          };
            }
            else
            {
                ...
                args = new List<IMetadataExpression>{
            new MetadataConstant(){ Type = _systemString, Value = null, },
            new MetadataConstant(){ Type = _systemInt, Value = ((int)MethodHashAttributeFlags.ForDependenceMethod), },
          };
            }
            var attr = new CustomAttribute() { Constructor = _methodHashAttributeCtor, Arguments = args, };
        ...
        public override void RewriteChildren(MethodBody methodBody)
        {
            ...
            Contract.Assume(_copier.OriginalFor.ContainsKey(method));
            var o = _copier.OriginalFor[method];
            var originalMethod = (IMethodDefinition)o;
            var keepEntireBody = _methodDefsToKeep.Contains(originalMethod.InternedKey);
            if (!keepEntireBody)
            {
                ...
                uint contractOffset;
                if (_contractOffsets.TryGetValue(originalMethod, out contractOffset))
                {
                    if (contractOffset != uint.MaxValue)
                    { // then it has a contract
