...
CREATE TABLE Methods (
  ...
  --FullNameHash BIGINT UNIQUE NOT NULL,
  ...
  InferredExprString NTEXT	-- Just the "ToString()", saved only in DEBUG mode
);
...
CREATE TABLE IdHashTimeToMethod (
  MethodIdHash BINARY(20) NOT NULL,
  Time DATETIME NOT NULL,
  MethodId BIGINT NOT NULL CONSTRAINT FK_IdHashTimeToMethod REFERENCES Methods(Id) ON DELETE CASCADE
);

CREATE UNIQUE INDEX HashTime ON IdHashTimeToMethod(MethodIdHash, Time);
