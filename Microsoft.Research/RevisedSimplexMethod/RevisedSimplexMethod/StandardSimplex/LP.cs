...
namespace Microsoft.Glee.Optimization
{
    ...
    public class LP : LinearProgramInterface
    {
        ...
        private void ObjectInvariant()
        {
            ...
            Contract.Invariant(_knownValues != null);
            Contract.Invariant(_costsOfknownValues != null);
            Contract.Invariant(_constraintCoeffs.Count == this.relations.Count);
            Contract.Invariant(_constraintCoeffs.Count == this.rightSides.Count);
        }
        ...
        private readonly SortedDictionary<int, double> _knownValues = new SortedDictionary<int, double>();
        private readonly Dictionary<int, double> _costsOfknownValues = new Dictionary<int, double>();

        private SortedDictionary<int, double> KnownValues
        {
            get { return _knownValues; }
        }
 ...
 double[] costs;
        ...
        private Microsoft.Glee.Optimization.Tableau _tableau;
        ...
        public double[] FeasibleSolution()
        {
            ...
            double[] ret = _tableau.GetSolution(_nVars);
        ...
        public double[] MinimalSolution()
        {
            ...
            else if (_status == Status.Feasible)
                DoStageTwo();

            if (_status == Status.Optimal)
                return ExtendVector(_tableau.GetSolution(_nVars));
            ...
            LeastSquareSolution()
        {
            ...
            Contract.Assume(_constraintCoeffs.Count > 0);


            //calculate matrix A row length
            int l = _constraintCoeffs[0].Length;
            int n = l;
            ...
            for (int i = 0; i < m; i++)
            {
                double[] coeff = _constraintCoeffs[i];
                //copy coefficients
                for (int j = 0; j < l; j++)
                    A[offset + j] = coeff[j];
        ...
        private double[] ExtendVector(double[] v)
        {
            if (_knownValues.Count > 0)
            {
        ...
        private void ShrinkCostsAndCoefficients()
        {
            int[] zeroColumns = new int[_nVars];
            if (LookForZeroColumns)
            {
                zeroColumns = new int[_nVars];
                for (int i = 0; i < _nVars; i++)
                {
                    foreach (double[] d in _constraintCoeffs)
                    {
                        if (d[i] != 0)
                        {
                            zeroColumns[i] = 1;
            ...
            {
                //remember old costs
                foreach (int i in _knownValues.Keys)
                    _costsOfknownValues[i] = this.costs[i];
                //fix the right sides
                foreach (KeyValuePair<int, double> kv in this.KnownValues)
                {
                    for (int i = 0; i < _constraintCoeffs.Count; i++)
                        this.rightSides[i] -= _constraintCoeffs[i][kv.Key] * kv.Value;
                }

                for (int i = 0; i < _constraintCoeffs.Count; i++)
                    _constraintCoeffs[i] = ShrinkVector(_constraintCoeffs[i]);
                ...
                _nSlaksAndSurpluses = _nArtificials = 0;
                for (int i = 0; i < _constraintCoeffs.Count; i++)
                {
                    if (this.rightSides[i] < 0)
                    {
                        this.rightSides[i] *= -1;
                        double[] cf = _constraintCoeffs[i];
        ...
        private double[] ShrinkVector(double[] v)
        {
            double[] ret = new double[v.Length - _knownValues.Count];
        ...
        public double GetMinimalValue()
        {
            ...
            if (this.Status == Status.Optimal)
            {
                Contract.Assume(_nVars <= costs.Length, "Should it be an object invariant?");
                return DotProduct(_tableau.GetSolution(_nVars), costs, _nVars) + CostOfKnowVariables();
            }
            return 0;
        }
        ...
        {
            double r = 0;
            foreach (KeyValuePair<int, double> kv in _knownValues)
                r += kv.Value * _costsOfknownValues[kv.Key];
            return r;
        }
        ...
        public void InitCosts(double[] costsParam)
        {
            ...
            if (_nVars == 0)
                _nVars = costsParam.Length;
            else
            {
                if (_nVars != costsParam.Length)
        ...
        private void CreateStageOneSolver()
        {
            Contract.Assume(_constraintCoeffs.Count > 0); // F: there should be at least 1 constraint

            int m = _constraintCoeffs.Count;
            int length1 = _nVars + _nSlaksAndSurpluses + _nArtificials;
            ...
            for (int i = 0; i < m; i++, ioffset += length1)
            {
                ...
                double[] coeff = (double[])_constraintCoeffs[i];
                ...
                if (r == Relation.LessOrEqual)
                {
                    ...
                    int j = _nVars + localSlacksAndSurps++;
                    X[ioffset + j] = 1;
                ...
                else if (r == Relation.GreaterOrEqual)
                {
                    ...
                    int artificial = _nVars + _nSlaksAndSurpluses + _nArtificials++;
            ...
            Contract.Assume(0 <= _nVars + _nSlaksAndSurpluses);
            for (int i = _nVars + _nSlaksAndSurpluses; i < artifCosts.Length; i++)
            {
                artifCosts[i] = 1;
            }
            _solver = new Solver(basis, X, x, artifCosts, EpsilonForReducedCosts);
        }
        /// <summary>
        /// make all coefficients not greater than 1, unless they are small aready
        /// </summary>
        private void ScaleToMaxVal()
        {
            if (_maxVal < 10)
                return;
            for (int i = 0; i < _constraintCoeffs.Count; i++)
            {
                double[] cc = _constraintCoeffs[i];
                for (int j = 0; j < cc.Length; j++)
                    cc[j] /= _maxVal;
            }

            for (int i = 0; i < rightSides.Count; i++)
                rightSides[i] /= _maxVal;
        }
        private double _epsilonForArtificials = 1.0E-8;
        ...
        private double _epsilonForReducedCosts = 1.0E-8;

        public double EpsilonForReducedCosts
        {
            get { return _epsilonForReducedCosts; }
            set { _epsilonForReducedCosts = value; }
        }
        ...
        private void DoStageOne()
        {
            ...
            _solver.ForbiddenPairs = _forbiddenPairs;
            _solver.Solve();
            if (_solver.status != Status.Optimal)
            {
                _status = Status.Infeasible;
                return;
            }
            ...
            _tableau = _solver.tableau;
            var X = _tableau.ReturnMatrix();
            var x = _tableau.GetSolution();
            var basis = _tableau.ReturnBasis();

            Contract.Assume(X != null);
            Contract.Assume(x != null);
            Contract.Assume(basis != null);

            int n = _tableau.ReturnLengthOne();

            int artificialsStart = _nVars + _nSlaksAndSurpluses;
            int ioffset = 0;
            ...
            for (int i = 0; i < x.Length; i++, ioffset += n)
            {
                ...
                if (basis[i] >= artificialsStart)
                {
                    ...
                    {
                        _status = Status.Infeasible;  //one of the artificials remains non-zero
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        _tableau.UpdateMatrix(X, x.Length, n);
                        _tableau.SetSolution(x);
                        _tableau.UpdateBasis(newBasis);
                        basis = newBasis;
                        i--;
                        ioffset -= n;
                    }
        ...
        public void Minimize()
        {
            ...
            if (_solver == null || _solver.status == Status.Unknown)
            {
        ...
        {
            var lengthWithNoArt = _nVars + _nSlaksAndSurpluses;
            var extendedCosts = new double[lengthWithNoArt];

            for (var i = 0; i < _nVars; i++)
            {
                extendedCosts[i] = costs[i];
            }
            _solver.SetCosts(extendedCosts);

            _solver.status = Status.Unknown;
            _solver.Solve();
